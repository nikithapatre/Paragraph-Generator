...
amount.innerHTML = inputValue;
// console.log(inputValue);
console.log(isNaN(inputValue)); 

const random = Math.floor(Math.random() * text.length);

// what if the input values are = empty, -1, >9
if(isNaN(inputValue) || inputValue <= 0 || inputValue > 9) {    //if all these are ture
    loremText.innerHTML = `<p>${text[random]}</p>`
} else {
    let tempText = text.slice(0, inputValue);   //to start from 0
    tempText = tempText
    .map((item) => {    // loop thru each array
   return `<p>${item}</p>`
    })
    .join(""); //joins all the paragraphs
...
//         console.log("numbers of paragraph")
//         else (inputValue > 9) 
